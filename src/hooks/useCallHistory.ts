...
export const useCallHistory = (options: UseCallHistoryOptions = {}) => {
    ...
    async (reset = false) => {
      ...
      try {
        ...
        if (filters.sentiment && processedCalls.length > 0) {
          filteredCalls = processedCalls.filter(
            (call) => call.analysis?.sentiment_analysis?.overall === filters.sentiment
          );
        }
  ...
  const exportToPDF = useCallback(
    async (callIds?: string[]) => {
      try {
        const callsToExport = callIds ? calls.filter((c) => callIds.includes(c.id)) : calls;
                  ...
                  <div class="summary">
                    ...
                    <ul>
                      ${call.analysis.action_items
                        .map((item) => `<li>${item.task} (${item.priority})</li>`)
                        .join('')}
                    </ul>
                  </div>
                `
