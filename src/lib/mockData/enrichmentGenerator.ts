...
function generateEmail(name: string, practiceName: string): string {
  ...
  const practiceSlug = practiceName
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '')
    .substring(0, 20);
...
export function enrichContact(baseContact: BaseContact): EnrichedContact {
  ...
  const overallRating =
    Math.round(
      ((googleRating * googleReviewCount +
        ...
        healthgradesRating * healthgradesReviewCount) /
        totalReviews) *
        100
    ) / 100;
  ...
  const baseCerts =
    CERTIFICATIONS[baseContact.specialization] || CERTIFICATIONS['General Dentistry'];
