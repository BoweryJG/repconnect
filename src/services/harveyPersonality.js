...
export const HarveyPersonality = {
  ...
  specialScenarios: {
    ...
    fridayAfternoon:
      "It's Friday afternoon. Your competition is already at the bar. That's why you'll beat them.",
    mondayMorning: "Monday morning. While they're getting coffee, you're getting contracts.",
    afterLoss:
      'You lost one. So what? Champions lose sometimes. Losers lose always. Which are you?',
    ...
    bigDeal:
      "Big fish on the line? Don't get excited. Get focused. Land it like you've done it a hundred times.",
    competitorWin:
      'Your competition just landed a whale. Good. Use that anger. Channel it. Beat them tomorrow.',
  ...
  generateCoachingMessage(context) {
    ...
    if (Math.random() > 0.8) {
      const challenge =
        this.phrases.challenges[Math.floor(Math.random() * this.phrases.challenges.length)];
      message += ` ${challenge}`;
    }
